Gitian building
================
*Setup instructions for a Gitian build of Revo Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Revo
...
- [Building Revo Core](#building-revo-core)
...
Clone the git repositories for revo and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/revolutionchain/revo --recursive
git clone https://github.com/revolutionchain/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/revo/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the revo repository under 'Fetch and create inputs' to install sources which require
...
To build Revo Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the revo repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/revo/.git/
...
For example:
```bash
URL=https://github.com/laanwj/revo.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit revo=${COMMIT} --url revo=${URL} ../revo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit revo=${COMMIT} --url revo=${URL} ../revo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit revo=${COMMIT} --url revo=${URL} ../revo/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-revo:
./bin/gbuild --commit revo=${COMMIT},cpp-eth-revo=${ETHCOMMIT} --url revo=${URL},cpp-eth-revo=${ETHURL} ../revo/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the revo git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../revo/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/revolutionchain/revo-detached-sigs.git

BTCPATH=/some/root/path/revo
SIGPATH=/some/root/path/revo-detached-sigs

./bin/gbuild --url revo=${BTCPATH},signature=${SIGPATH} ../revo/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/revo-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/revo-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/revo-osx-build.assert
```
...
[revolutionchain/gitian.sigs](https://github.com/revolutionchain/gitian.sigs/) repository, or if that's not possible create a pull
request. 
