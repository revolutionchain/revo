...
The data directory is the default location where the Revo Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.revo/`
macOS    | `$HOME/Library/Application Support/Revo/`
Windows  | `%APPDATA%\Revo\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `revo.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Revo blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `banlist.dat`         | Stores the addresses/subnets of banned nodes (deprecated). `revod` or `revo-qt` no longer save the banlist to this file, but read it on startup if `banlist.json` is not present.
`./`               | `banlist.json`        | Stores the addresses/subnets of banned nodes.
`./`               | `revo.conf`           | User-defined [configuration settings](bitcoin-conf.md) for `revod` or `revo-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `revod.pid`           | Stores the process ID (PID) of `revod` or `revo-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `revod` or `revo-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [revo.conf](bitcoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`revo-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Revo Core:
