...
This guide describes how to build revod, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Revo Core from source.
...
### 4. Clone Revo repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Revo Core repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone --recursive https://github.com/revoproject/revo.git
```
...
It is not necessary to build wallet functionality to run `revod` or  `revo-qt`.
...
Revo Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Revo Core application using `make deploy`.
...
There are many ways to configure Revo Core, here are a few common examples:
...
Run the following in your terminal to compile Revo Core:
...
Revo Core should now be available at `./src/revod`.
If you compiled support for the GUI, it should be available at `./src/qt/revo-qt`.

The first time you run `revod` or `revo-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Revo/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Revo"

touch "/Users/${USER}/Library/Application Support/Revo/revo.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Revo/revo.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Revo/debug.log
```

## Other commands:

```shell
./src/revod -daemon      # Starts the revo daemon.
./src/revo-cli --help    # Outputs a list of command-line options.
./src/revo-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/revo-qt -server # Starts the revo-qt server mode, allows revo-cli control
```
