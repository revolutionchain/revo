 ...
 */
static const char* BITCOIN_PID_FILENAME = "revod.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("revo-shutoff");
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-forceibd", strprintf("Force initial blocks download mode for the node (default: %u)", DEFAULT_FORCE_INITIAL_BLOCKS_DOWNLOAD_MODE), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/revolutionchain/revo>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(datadir / "stateRevo");
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if revo is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if revo is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path revoStateDir = gArgs.GetDataDirNet() / "stateRevo";
                bool fStatus = fs::exists(revoStateDir);
                const std::string dirRevo(revoStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsRevostate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<RevoState>(new RevoState(dev::u256(0), RevoState::openDB(dirRevo, hashDB, dev::WithExisting::Trust), dirRevo, existsRevostate));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(revoStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setRevoSchedule(revoDGP.getGasSchedule(active_chain.Height() + (active_chain.Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
