...
public:
    ...
    bool isUnspentAddress(const std::string &revoAddress) override
    {
        LOCK(m_wallet->cs_wallet);
        ...
        for (const COutput& out : vecOutputs)
        {
            ...
            if(fValidAddress && EncodeDestination(address) == revoAddress && out.tx->tx->vout[out.i].nValue)
            {
                return true;
            }
        }
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(std::holds_alternative<PKHash>(dest))
            {
                ...
                contractRet = m_revoDelegation.ExistDelegationContract() ? m_revoDelegation.GetDelegation(address, delegation, m_wallet->chain().chainman().ActiveChainstate()) : false;
                if(contractRet)
                {
                    validated = m_revoDelegation.VerifyDelegation(address, delegation);
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        if(std::holds_alternative<PKHash>(dest))
        {
            PKHash keyID = std::get<PKHash>(dest);
            uint160 address(keyID);
            details.c_contract_return = m_revoDelegation.ExistDelegationContract() ? m_revoDelegation.GetDelegation(address, delegation, m_wallet->chain().chainman().ActiveChainstate()) : false;
            if(details.c_contract_return)
            {
                details.c_entry_exist = m_revoDelegation.VerifyDelegation(address, delegation);
            ...
            if(decoded_psbt.tx->HasOpCall())
            {
                ...
                for(size_t i = 0; i < decoded_psbt.tx->vout.size(); i++){
                    ...
                    if(v.scriptPubKey.HasOpCall()){
                        ...
                        if(RevoDelegation::IsAddBytecode(data))
                        {
                            std::string hexStaker;
                            if(!RevoDelegation::GetUnsignedStaker(data, hexStaker))
                            {
    ...
    bool setAddDelegationData(std::string& psbt, const std::map<int, SignDelegation>& signData, std::string& error) override
    {
        ...
        for (auto it = signData.begin(); it != signData.end(); it++)
        {
            ...
            if(v.scriptPubKey.HasOpCall()){
                ...
                v.scriptPubKey.GetData(data);
                CScript scriptRet;
                if(RevoDelegation::SetSignedStaker(data, PoD) && v.scriptPubKey.SetData(data, scriptRet))
                {
    ...
    RevoDelegation m_revoDelegation;
};
