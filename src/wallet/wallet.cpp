...
bool CWallet::IsFullySpentAtDepth(const uint256& hash, unsigned int depth) const
{
    ...
    if (mi != mapWallet.end())
    {
        const CWalletTx* pwtx = &(mi->second);
        const int outs_total = pwtx->tx->vout.size();
        for (int i = 0; i < outs_total; i++)
        {
            // check if empty
            if (pwtx->tx->vout[i].IsEmpty())
            {
                continue;
            }

            // check if unspendable
            if (pwtx->tx->vout[i].scriptPubKey.IsUnspendable())
            {
                continue;
            }
...
void CWallet::updatedBlockTipEx(const CBlockIndex* pindex)
{
    if (fTxDeleteEnabled && pindex && (pindex->nHeight % 5 == 0)) // delete transactions once in five new blocks
    {
        ...
        if (m_chain->isInitialBlockDownload() || pindex->GetBlockTime() < GetTimeSeconds() - 144 * 60)
        {
...
void CWallet::ReorderWalletTransactions(std::map<std::pair<int,int>, const uint256> &mapSorted, int64_t &maxOrderPos)
{
    ...
    int maxSortNumber = m_chain->getHeight().value() + 1;
    ...
    for (const auto &it : mapWallet)
    {
        ...
        {
            int wtxHeight = 0;
            m_chain->findBlock(pwtx->m_confirm.hashBlock, FoundBlock().height(wtxHeight));

            if (wtxHeight)
            {
                auto key = std::make_pair(wtxHeight, pwtx->m_confirm.nIndex);
                mapSorted.insert(std::make_pair(key, wtxid));
            }
            else
            {
                // wtx block not found in chain !? 
            }
        }
        else
        {
...
void CWallet::UpdateWalletTransactionOrder(std::map<std::pair<int,int>, const uint256> &mapSorted, bool resetOrder)
{
    ...
    for (const auto &it : mapUpdatedPos)
    {
        ...
        WalletBatch(GetDatabase(), false).WriteTx(itmw->second);
    }

    // Update Next Wallet Tx Positon
    nOrderPosNext = previousPosition++;
    WalletBatch(GetDatabase(), false).WriteOrderPosNext(nOrderPosNext);
...
unsigned int CWallet::DeleteTransactions(std::vector<uint256> &removeTxs)
{
    ...
    for (const uint256& txid_to_delete : removeTxs)
    {
        ...
        {
            if (WalletBatch(GetDatabase()).EraseTx(txid_to_delete))
            {
                nRemoved++;
...
void CWallet::StakeRevos(bool fStake)
{
    ::StakeRevos(fStake, this, stakeThread);
}
...
void CWallet::StartStake()
{
    if(IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
    {
        m_enabled_staking = !m_ledger_id.empty() && RevoLedger::instance().toolExists();
    }
    else
    {
