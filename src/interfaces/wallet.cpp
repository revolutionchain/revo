...
public:
    ...
    DelegationInfo getDelegationContract(const std::string &sHash, bool& validated, bool& contractRet) override
    {
        ...
        if (mi != m_wallet->mapDelegation.end()) {
            ...
            if(keyID)
            {
                ...
                contractRet = m_revoDelegation.ExistDelegationContract() ? m_revoDelegation.GetDelegation(address, delegation) : false;
                if(contractRet)
                {
    ...
    DelegationDetails getDelegationDetails(const std::string &sAddress) override
    {
        ...
        {
            uint160 address(*keyID);
            details.c_contract_return = m_revoDelegation.ExistDelegationContract() ? m_revoDelegation.GetDelegation(address, delegation) : false;
