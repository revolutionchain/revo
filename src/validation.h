...
#include <revo/revoDGP.h>
#include <libethereum/ChainParams.h>
#include <libethereum/LastBlockHashesFace.h>
#include <libethashseal/GenesisInfo.h>
#include <script/standard.h>
#include <revo/storageresults.h>


extern std::unique_ptr<RevoState> globalState;
...
using ExtractRevoTX = std::pair<std::vector<RevoTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // revo
...
/** Minimum depth for wtx we will accept to delete. */
static const unsigned int MIN_DELETETX_DEPTH = 1000 - 1;
...
class RevoTxConverter{

public:

    RevoTxConverter(CTransaction tx, CChainState& _chainstate, const CTxMemPool* _mempool, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL, unsigned int flags = SCRIPT_EXEC_BYTE_CODE) : txBit(tx), view(v), blockTransactions(blockTxs), sender(false), nFlags(flags), chainstate(_chainstate), mempool(_mempool){}

    bool extractionRevoTransactions(ExtractRevoTX& revoTx);
...
private:
    ...
    RevoTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<RevoTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex, CChain& _chain) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex), chain(_chain) {}
