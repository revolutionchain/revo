...
const char * const BITCOIN_CONF_FILENAME = "revo.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Revo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Revo
    // Mac: ~/Library/Application Support/Revo
    // Unix: ~/.revo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Revo";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Revo";
#else
    // Unix
    return pathRet / ".revo";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Revo Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Revo Core Developers";
    }
