...
using namespace RevoLedger_NS;

class RevoLedgerPriv
{
public:
    RevoLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("RevoLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
RevoLedger::RevoLedger():
    d(0)
{
    d = new RevoLedgerPriv();
}
...
RevoLedger &RevoLedger::instance()
{
    static RevoLedger device;
    return device;
}

bool RevoLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool RevoLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool RevoLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool RevoLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool RevoLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool RevoLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool RevoLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string RevoLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool RevoLedger::toolExists()
{
    return d->toolExists;
}

bool RevoLedger::isStarted()
{
    return d->fStarted;
}

void RevoLedger::wait()
{
    if(d->fStarted)
    {
...
bool RevoLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool RevoLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool RevoLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool RevoLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool RevoLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool RevoLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "Revo Stake" || device.app_name == "Revo Stake Test";
        if(isStakeApp == stake)
        {
...
bool RevoLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool RevoLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool RevoLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool RevoLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string RevoLedger::derivationPath(int type)
{
