...
void RevoDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool RevoDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
uint64_t RevoDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
bool RevoDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void RevoDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void RevoDGP::initStorageTemplate(const dev::Address& addr){
...
dev::Address RevoDGP::getAddressForBlock(unsigned int blockHeight){
...
void RevoDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RevoDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void RevoDGP::parseStorageOneUint64(uint64_t& value){
...
void RevoDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule RevoDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
