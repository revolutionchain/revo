...
ResultExecute RevoState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, RevoTransaction const& _t, CChain& _chain, Permanence _p, OnOpFunc const& _onOp){
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            RevoTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                RevoTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
