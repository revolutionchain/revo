...
#include <qt/revohwitool.h>
#include <qt/guiutil.h>
#include <qt/execrpccommand.h>
#include <qt/walletmodel.h>
#include <util/strencodings.h>
#include <util/system.h>
#include <revo/revoledger.h>
...
class RevoHwiToolPriv
{
public:
    RevoHwiToolPriv(QObject *parent)
    {
...
RevoHwiTool::RevoHwiTool(QObject *parent) : QObject(parent)
{
    d = new RevoHwiToolPriv(this);
}

RevoHwiTool::~RevoHwiTool()
{
    delete d;
}

bool RevoHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(RevoLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(RevoLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool RevoHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = RevoLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(RevoLedger::instance().errorMessage());
    return ret;
}

bool RevoHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = RevoLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool RevoHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool RevoHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool RevoHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool RevoHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool RevoHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = RevoLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(RevoLedger::instance().errorMessage());
    return ret;
}

bool RevoHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = RevoLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(RevoLedger::instance().errorMessage());
    return ret;
}

bool RevoHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString RevoHwiTool::errorMessage()
{
...
bool RevoHwiTool::isStarted()
{
    return d->fStarted;
}

void RevoHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool RevoHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool RevoHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool RevoHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool RevoHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool RevoHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool RevoHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void RevoHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool RevoHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void RevoHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString RevoHwiTool::derivationPathPKH()
{
    std::string path = RevoLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString RevoHwiTool::derivationPathP2SH()
{
    std::string path = RevoLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString RevoHwiTool::derivationPathBech32()
{
    std::string path = RevoLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
