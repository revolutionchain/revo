...
Token::Token()
{
    ...
    lstMandatory.append(RevoToken::paramAddress());
    lstMandatory.append(RevoToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(RevoToken::paramSender());
    ...
    lstMandatory.append(RevoToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(RevoToken::paramAmount());
    lstOptional.append(RevoToken::paramGasLimit());
    lstOptional.append(RevoToken::paramGasPrice());
    lstOptional.append(RevoToken::paramSender());
    lstOptional.append(RevoToken::paramBroadcast());
    lstOptional.append(RevoToken::paramChangeToSender());
    lstOptional.append(RevoToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToRevoAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToRevoAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
