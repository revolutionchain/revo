...
bool ToHash160(const std::string& strRevoAddress, std::string& strHash160)
{
    CTxDestination revoAddress = DecodeDestination(strRevoAddress);
    if(!IsValidDestination(revoAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&revoAddress);
    if(keyid){
...
bool ToRevoAddress(const std::string& strHash160, std::string& strRevoAddress)
{
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToRevoAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToRevoAddress(tokenEvent.receiver, tokenEvent.receiver);
