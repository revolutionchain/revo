...
#define paternVersion "revo-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

RevoVersionChecker::RevoVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

RevoVersionChecker::~RevoVersionChecker()
{

}

bool RevoVersionChecker::newVersionAvailable()
{
...
Version RevoVersionChecker::getMaxReleaseVersion()
{
