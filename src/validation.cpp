...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        RevoDGP revoDGP(globalState.get(), m_active_chainstate, fGettingValuesDGP);
        uint64_t minGasPrice = revoDGP.getMinGasPrice(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        uint64_t blockGasLimit = revoDGP.getBlockGasLimit(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        size_t count = 0;
        ...
        RevoTxConverter converter(tx, m_active_chainstate, &m_pool, NULL, NULL, contractflags);
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s RVO additional fees, %d delta bytes\n",
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CChainState& chainstate, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    RevoDGP revoDGP(globalState.get(), chainstate, fGettingValuesDGP);
    uint64_t blockGasLimit = revoDGP.getBlockGasLimit(chainstate.m_chain.Tip()->nHeight + 1);
    ...
    {
        callTransaction = RevoTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = RevoTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<RevoTransaction>(1, callTransaction), blockGasLimit, pblockindex, chainstate.m_chain);
...
void writeVMlog(const std::vector<ResultExecute>& res, CChain& chain, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path revoDir = gArgs.GetDataDirNet() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
    ...
    for(RevoTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                RevoTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // revo
    RevoDGP revoDGP(globalState.get(), *this, fGettingValuesDGP);
    globalSealEngine->setRevoSchedule(revoDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = revoDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = revoDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = revoDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
...
///////////////////////////////////////////////////////////////////////////////////////// revo
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            RevoTxConverter convert(tx, *this, m_mempool, &view, &block.vtx, contractflags);
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertRevoTX.first, blockGasLimit, pindex->pprev, m_chain);
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    RevoLedger &device = RevoLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet)
{
    ...
    {
        std::string errorMessage = RevoLedger::instance().errorMessage();
...
CAmount GetTxGasFee(const CMutableTransaction& _tx, const CTxMemPool& mempool, CChainState& active_chainstate)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        RevoTxConverter convert(tx, active_chainstate, &mempool, NULL, NULL, contractflags);
