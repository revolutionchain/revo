...
void checkResult(bool isCreation, std::vector<RevoTransaction> results, uint256 hash){
...
void runTest(CChainState& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    RevoTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractRevoTX revoTx;
    BOOST_CHECK(converter.extractionRevoTransactions(revoTx));
    std::vector<RevoTransaction> result = revoTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(revotxconverter_tests, TestingSetup)
