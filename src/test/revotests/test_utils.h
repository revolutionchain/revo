...
inline void initState(){
    ...
    const std::string dirRevo = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<RevoState>(new RevoState(dev::u256(0), RevoState::openDB(dirRevo, hashDB, dev::WithExisting::Trust), dirRevo + "/revoDB", dev::eth::BaseState::Empty));
...
inline dev::Address createRevoAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline RevoTransaction createRevoTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    RevoTransaction txEth;
    if(recipient == dev::Address()){
        txEth = RevoTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = RevoTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
