...
#include <revotests/precompiled_utils.h>
#include <test/revotests/data/btc_ecrecover.json.h>
#include <test/revotests/data/ecrecover.json.h>
#include <test/revotests/data/sha256.json.h>
#include <test/revotests/data/ripemd160.json.h>
#include <test/revotests/data/identity.json.h>
#include <test/revotests/data/modexp.json.h>
#include <test/revotests/data/modexp_eip2565.json.h>
#include <test/revotests/data/alt_bn128_G1_add.json.h>
#include <test/revotests/data/alt_bn128_G1_mul.json.h>
#include <test/revotests/data/alt_bn128_pairing_product.json.h>
#include <test/revotests/data/blake2_compression.json.h>
...
BOOST_AUTO_TEST_CASE(checking_london_after_fork){
    ...
    txs.push_back(createRevoTransaction(getCode(CodeID::contract), 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs, *m_node.chainman);

    {
        // Call basefee
        assert(txs.size() > 0);
        dev::Address proxy = createRevoAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<RevoTransaction> txIsItLondon;
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::getBaseFee), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...

    {
        ...
        dev::Address proxy = createRevoAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<RevoTransaction> txIsItLondon;
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::deploy1ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::deploy2ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::deploy3ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::deploy32ef), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::deploy1fe), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::call1ef), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::call2ef), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
...

    {
        ...
        dev::Address proxy = createRevoAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<RevoTransaction> txIsItLondon;
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::getStore), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::getLoad), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
        BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);
    ...
    for(size_t a = 1; a < 300; a++)
    {
        ...
        dev::Address proxy = createRevoAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<RevoTransaction> txIsItLondon;
        txIsItLondon.push_back(createRevoTransaction(data, 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
...

    {
        ...
        dev::Address proxy = createRevoAddress(txs[0].getHashWith(), txs[0].getNVout());
        std::vector<RevoTransaction> txIsItLondon;
        txIsItLondon.push_back(createRevoTransaction(getCode(CodeID::close), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
        auto result = executeBC(txIsItLondon, *m_node.chainman);
