...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    //////////////////////////////////////////////////////// revo
    RevoDGP revoDGP(globalState.get(), m_chainstate, fGettingValuesDGP);
    globalSealEngine->setRevoSchedule(revoDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = revoDGP.getBlockSize(nHeight);
    minGasPrice = revoDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = revoDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice, CBlock* pblock) {
    ...
    RevoTxConverter convert(iter->GetTx(), m_chainstate, &m_mempool, NULL, &pblock->vtx, contractflags);

    ExtractRevoTX resultConverter;
    if(!convert.extractionRevoTransactions(resultConverter)){
    ...
    for(RevoTransaction revoTransaction : revoTransactions){
        ...
        if(bceResult.usedGas + revoTransaction.gas() > softBlockGasLimit){
        ...
        if(revoTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, revoTransactions, hardBlockGasLimit, m_chainstate.m_chain.Tip(), m_chainstate.m_chain);
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            revoDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
            delegations_staker = revoDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                revoDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                revoDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            revoDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            revoDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    RevoDelegation revoDelegations;
    int32_t cacheHeight;
    ...
    void Update(int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                revoDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
                pwallet->m_my_delegations = revoDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    revoDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                    revoDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                revoDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(revoDelegations.GetDelegation(address, delegation, pwallet->chain().chainman().ActiveChainstate()) && RevoDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "revostake";
...
void StakeRevos(bool fStake, CWallet *pwallet, boost::thread_group*& stakeThread)
{
